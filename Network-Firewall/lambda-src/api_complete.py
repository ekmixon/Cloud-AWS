...
class APIHarness:
    ...
    def __init__(self, creds, base_url="https://api.crowdstrike.com"):
        ...
        self.commands = [
            ...
            ["RTR-CheckActiveResponderCommandStatus", "GET",
    ...
    def command(self, action="", parameters={}, body={}, data={}, ids=False, override=False):
        ...
        if CMD:
            ...
            if self.authenticated:
                try:
                    response = requests.request(CMD[0][1].upper(), FULL_URL, json=BODY, data=DATA, params=PARAMS,
                                                headers=HEADERS, verify=False)
                    returned = self.Result()(status_code=response.status_code, headers=response.headers,
                                             body=response.json())
                except Exception as e:
                    returned = self.Result()(status_code=500, headers={}, body=str(e))
            else:
                returned = self.Result()(status_code=500, headers={},
